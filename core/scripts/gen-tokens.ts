...
note("Semantic surface");
v("surface-hover", color.surfaceHover);
v("scrim", color.scrim);
...
for (const [c, a] of Object.entries(accent)) {
  ...
  v(`accent-${c}-muted`, a.muted); // 흰색에 가깝게 탁한 틴트 (table strip 등)
}
blank();
...
for (const [r, c] of Object.entries(roleColor)) {
  ...
  v(`accent-${r}-muted`, `var(${PREFIX}accent-${c}-muted)`);
}
blank();
// 컨텍스트 accent (계층 전파) — 컴포넌트/컨테이너가 .funky-accent--{color} 로 현재 accent 를
// 선언하면 하위/상태(hover·strip·fill)가 var(--funky-ac-*) 로 그 색을 따라간다. 기본은 neutral.
note("Contextual accent — neutral 기본 (.funky-accent--* 가 덮음)");
v("ac-bg", "var(--funky-ink)"); //          solid accent 면 (table 헤더·활성 등)
v("ac-fg", "var(--funky-ink-inverse)"); //   그 면 위 글자
v("ac-hover", "var(--funky-surface-hover)"); //은은한 hover 틴트 (neutral=회색)
v("ac-strip", "var(--funky-sunken)"); //     zebra strip (neutral=크림)
v("ac-text", "var(--funky-ink)"); //         accent 글자/테두리
blank();
...
// ───── 컨텍스트 accent setter 클래스 — 현재 accent 를 선언(.funky-accent--{color|role}) ──
const accentSetters = [...Object.keys(accent), ...Object.keys(roleColor)]
  .map(
    (name) =>
      `.funky-accent--${name} {\n` +
      `  --funky-ac-bg: var(${PREFIX}accent-${name}-solid-bg);\n` +
      `  --funky-ac-fg: var(${PREFIX}accent-${name}-solid-fg);\n` +
      `  --funky-ac-hover: var(${PREFIX}accent-${name}-muted);\n` +
      `  --funky-ac-strip: var(${PREFIX}accent-${name}-muted);\n` +
      `  --funky-ac-text: var(${PREFIX}accent-${name}-soft-fg);\n}`,
  )
  .join("\n");
...
/* ── Contextual accent setters — .funky-accent--{color|role} 선언 시 하위가 var(--funky-ac-*) 로 전파 ── */
${accentSetters}
`;
