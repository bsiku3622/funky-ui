...
본문을 제목 + 얇은 가로줄 + 흰 여백으로 흘려보내면 docs 리더가 됩니다. 같은 콘텐츠를 `Panel`(검정/네온 헤더바 + 2px 테두리 + 하드 그림자)에 담으면 funky-ui가 됩니다.
...
- **Window dots** — 코드·미리보기·export 샷은 `Window`(신호등 dots 타이틀바 + 2px 프레임 + 하드 그림자)로 감쌉니다.
